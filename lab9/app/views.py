...
from app.forms import LoginForm, FeedbackForm, RegistrationForm,UpdateAccountForm, UpdatePasswordForm
...
@login_required
def account():
    update_info_form = UpdateAccountForm()
    update_password_form = UpdatePasswordForm()

    if update_info_form.validate_on_submit():
        update_user_info(update_info_form)
        flash('Інформацію про ваш обліковий запис оновлено!', 'success')
        return redirect(url_for('account'))

    if request.method == 'GET':
        update_info_form.username.data = current_user.username
        update_info_form.email.data = current_user.email
        update_info_form.about_me.data = current_user.about_me

    image_file = url_for('static', filename='profile_pics/' + current_user.image_file)

    if update_password_form.validate_on_submit():
        if bcrypt.check_password_hash(current_user.password, update_password_form.old_password.data):
            ...
            flash('Старий пароль введено невірно. Пароль не оновлено.', 'danger')
            return redirect(url_for('account'))

    return render_template('account.html', title='Account', image_file=image_file, update_info_form=update_info_form, update_password_form=update_password_form)


def update_user_info(update_info_form):
    if update_info_form.picture.data:
        picture_file = save_picture(update_info_form.picture.data)
        current_user.image_file = picture_file

    if update_info_form.username.data:
        current_user.username = update_info_form.username.data

    if update_info_form.email.data:
        current_user.email = update_info_form.email.data

    if update_info_form.about_me.data:
        current_user.about_me = update_info_form.about_me.data
...
def update_password():
    ...
    if update_password_form.validate_on_submit():
        ...
        flash('Паролі не співпадають. Пароль не оновлено.', 'danger')
