...
def handle_client(conn, addr, users):
    ...
    users.append(conn)
    print(f'Користувач {os.getlogin()} з IP: {addr} котрий підключився до сервера в [{connection_time}]\
        \nКористувачі онлайн: {len(users)}')
    while True:
        data = conn.recv(1024)
        if not data:
            users.remove(conn)
            print(f"Користувач {os.getlogin()} з IP: {addr} котрий відключився від сервера\
                \nКористувачі онлайн: {len(users)}")
            break
