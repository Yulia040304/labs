...
class PortfolioViewsTests(BaseTestCase):
    def test_home_page_loads(self):
        ...
        THEN check that status code is 200 and response data contains 'Flask microframework for web applications'
        """

        with self.client:
            response = self.client.get(url_for('portfolio.home'))
            self.assertEqual(response.status_code, 200)
            self.assertIn('Python-Flask', response.data.decode('utf-8'))

    def test_about_page_loads(self):
        """
        GIVEN url to about page
        WHEN the '/about' page is requested (GET)
        THEN check that status code is 200 and response data contains 'Hello, this is my biography.'
        """

        with self.client:
            response = self.client.get(url_for('portfolio.about'))
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'A more detailed biography can be found below', response.data)

    def test_page1_loads(self):
        """
        GIVEN url to page1
        WHEN the '/page1' page is requested (GET)
        THEN check that status code is 200 and response data contains 'Сторінка 1'
        """
    ...
    def test_page2_loads(self):
        """
        GIVEN url to page2
        WHEN the '/page2' page is requested (GET)
        THEN check that status code is 200 and response data contains 'Сторінка 2'
        """
    ...
    def test_page3_loads(self):
        """
        GIVEN url to page3
        WHEN the '/page3' page is requested (GET)
        THEN check that status code is 200 and response data contains 'Сторінка 3'
        """
